...
export const lisCategories = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/categories/all`, config);
...
export const editCategory = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/categories/${id}`);
    dispatch({ type: CATEGORY_EDIT_SUCCESS, payload: data });
  } catch (error) {
    const message =
      error.response && error.response.data.message
